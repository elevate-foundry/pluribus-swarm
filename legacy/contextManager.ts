 ...
 * while preserving swarm personality and understanding
 */
...
Be extremely concise but preserve meaning. Use symbolic language where possible.`
    ...
    return `[Compressed: ${messages.length} messages from earlier in the conversation]`;
  }
}
...
function identifyAnchors(messages: Message[]): Set<number> {
  ...
  messages.forEach((msg, idx) => {
    ...
    // Important revelations or questions
    if (content.includes('why') || content.includes('how') || content.includes('what is')) {
      if (content.length > 100) { // Substantial questions
        anchors.add(idx);
      }
    }
  });
...
export async function manageContext(
  messages: Message[],
  maxTokens: number = 6000, // Conservative limit
...
export function getContextStats(messages: Message[], maxTokens: number = 8000): {
  ...
  return {
    ...
    canFitMore: totalTokens < maxTokens * 0.9, // 90% threshold
    remainingTokens
  };
}
